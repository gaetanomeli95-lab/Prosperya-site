...
export default function Internazionalizzazione() {
  return (
    <div className="premium-page">
      ...
      <div className="absolute left-[-9rem] top-24 h-[30rem] w-[30rem] rounded-full bg-mediterranean/13 blur-3xl" />
      ...
      <div className="premium-container">
        <section className="premium-panel-dark relative overflow-hidden p-6 sm:p-9 lg:p-12 xl:p-14">
          ...
          <div className="relative grid gap-10 lg:grid-cols-12 lg:items-end">
            <div className="lg:col-span-8">
              <FadeIn>
                <div className="flex items-center gap-3"><span className="h-px w-8 bg-sand" /><span className="eyebrow !text-white/40">Cross-border advisory</span></div>
            ...
            <div className="lg:col-span-4">
              <FadeIn delay={0.1}>
                <p className="max-w-lg text-sm leading-[1.8] !text-white/60 sm:text-base">{home.international.text}</p>
              </FadeIn>
            </div>
          </div>
        </section>
        ...
        <section className="mt-5 grid gap-4 lg:grid-cols-[.8fr_1.2fr]">
          <div className="space-y-4">
            {[
              ...
              <FadeIn key={title} delay={i * 0.06}>
                <article className={`premium-card p-6 sm:p-8 ${i === 1 ? '!border-night !bg-night' : ''}`}>
                  <div className="flex items-start justify-between gap-4">
                    <span className={`font-heading italic text-2xl ${i === 1 ? 'text-sand' : 'text-mediterranean'}`}>{n}</span>
                    <span className={`text-[9px] font-semibold uppercase tracking-[.2em] ${i === 1 ? '!text-white/30' : 'text-night/30'}`}>International desk</span>
                  </div>
                  <h2 className={`mt-10 text-3xl font-heading leading-[1] ${i === 1 ? '!text-white' : 'text-night'}`}>{title}</h2>
                  <p className={`mt-5 text-sm leading-[1.75] ${i === 1 ? '!text-white/55' : 'text-anthracite/67'}`}>{text}</p>
                </article>
              </FadeIn>
            ))}
          </div>
          ...
          <FadeIn delay={0.12}>
            <div className="premium-panel-dark relative overflow-hidden p-4 sm:p-7 lg:p-10">
              <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-sand/80 to-transparent" />
              <div className="mb-6 flex items-center justify-between gap-4">
                <div>
                  <p className="eyebrow !text-white/32">Operating network</p>
                  <h2 className="mt-3 text-3xl font-heading !text-white sm:text-4xl">Europa · Mediterraneo</h2>
                </div>
                <span className="hidden text-[9px] font-semibold uppercase tracking-[.2em] !text-white/25 sm:block">Sicilia come hub</span>
              </div>
              <MapNetwork />
            </div>
          </FadeIn>
        ...
        <section className="mt-5 grid gap-4 lg:grid-cols-[1.2fr_.8fr]">
          <div className="premium-hero p-7 sm:p-9 lg:p-12">
            <p className="eyebrow text-night/36">Geografie operative</p>
            <div className="mt-8 grid gap-4 sm:grid-cols-3">
              {[
                ...
                <div key={label} className="border-t border-night/12 pt-5">
          ...
          <div className="premium-panel-dark flex flex-col justify-between p-7 sm:p-9 lg:p-10">
            <div>
