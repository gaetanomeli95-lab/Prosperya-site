...
export function Header() {
  ...
  const navClass = (href: string) => href === '/contatti/'
    ? 'group inline-flex items-center gap-2 whitespace-nowrap bg-white px-4 py-2.5 text-[13px] font-semibold !text-night transition-colors hover:bg-sand'
    : 'relative whitespace-nowrap text-[13px] font-medium !text-white/80 transition-colors hover:!text-white after:absolute after:-bottom-2 after:left-0 after:h-px after:w-0 after:bg-sand after:transition-all hover:after:w-full';
        ...
        <div className={`relative flex h-16 items-center justify-between border border-white/12 bg-[#090D0E]/95 px-3 backdrop-blur-xl transition-all duration-500 sm:px-4 xl:h-[72px] xl:px-6 ${scrolled ? 'shadow-float' : 'shadow-[0_12px_40px_rgba(0,0,0,0.16)]'}`}>
          <Link href="/" className="group flex min-w-0 items-center gap-3" aria-label={company.ragioneSociale}>
            <span className="relative grid h-10 w-10 shrink-0 place-items-center overflow-hidden rounded-full bg-white shadow-sm ring-1 ring-white/20 xl:h-11 xl:w-11">
              <Image src="/brand/prosperya-logo.png" alt="Prosperya" width={44} height={44} className="h-full w-full object-contain scale-[1.08]" priority />
            </span>
          ...
          <nav className="hidden items-center gap-4 xl:flex 2xl:gap-6" aria-label="Navigazione principale">
            {mainNav.map((item) => (
              <Link key={item.href} href={item.href} className={navClass(item.href)}>
                {item.label}
                {item.href === '/contatti/' && <ArrowUpRight className="h-4 w-4" />}
              </Link>
            ))}
          </nav>

          <button onClick={() => setOpen(!open)} className="grid h-10 w-10 shrink-0 place-items-center border border-white/20 bg-white/5 !text-white xl:hidden" aria-expanded={open} aria-label={open ? 'Chiudi menu' : 'Apri menu'}>
            ...
            <nav className="flex flex-col px-5 py-4" aria-label="Menu mobile">
              {mainNav.map((item) => (
                <Link
                  ...
                  className={item.href === '/contatti/'
                    ? 'my-3 inline-flex min-h-12 items-center justify-between bg-white px-4 py-3.5 text-sm font-semibold !text-night'
                    : 'border-b border-white/10 py-4 text-[15px] font-medium !text-white/90'}
                >
                  {item.label}
                  {item.href === '/contatti/' && <ArrowUpRight className="h-4 w-4" />}
                </Link>
