        ...
        <div className="mx-auto max-w-[1480px] px-3 sm:px-6 lg:px-8">
          <div className={`relative flex h-16 items-center justify-between border border-white/12 bg-[#090D0E]/95 px-3 backdrop-blur-xl transition-all duration-500 sm:px-4 xl:h-[72px] xl:px-6 ${scrolled ? 'shadow-float' : 'shadow-[0_12px_40px_rgba(0,0,0,0.16)]'}`}>
            ...
            <nav className="hidden items-center gap-4 xl:flex 2xl:gap-6" aria-label="Navigazione principale">
              {mainNav.map((item) => item.href === '/contatti/' ? (
                <button
                  key={item.href}
                  type="button"
                ...
                <Link key={item.href} href={item.href} className="relative whitespace-nowrap text-[13px] font-medium !text-white/80 transition-colors hover:!text-white after:absolute after:-bottom-2 after:left-0 after:h-px after:w-0 after:bg-sand after:transition-all hover:after:w-full">
                  {item.label}
                </Link>
              ))}
            </nav>
              ...
              <nav className="flex flex-col px-5 py-4" aria-label="Menu mobile">
                {mainNav.map((item) => item.href === '/contatti/' ? (
                  <button
                    ...
                    className="my-3 inline-flex min-h-12 items-center justify-between bg-white px-4 py-3.5 text-sm font-semibold !text-night"
                  >
                    {item.label}
                  ...
                  <Link
                    ...
                    className="border-b border-white/10 py-4 text-left text-[15px] font-medium !text-white/90"
                  >
                    {item.label}
                  </Link>
                ))}
